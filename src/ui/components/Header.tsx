					...
					<div className="flex h-full flex-col">
						...
						<div className="flex-1 overflow-auto p-4">
							...
							<nav className="flex items-center gap-4 md:hidden">
								<button className="font-tactical text-[13px] uppercase tracking-[0.08em] text-silver transition hover:text-white">
									КАТАЛОГ
								</button>
								<button className="font-tactical text-[13px] uppercase tracking-[0.08em] text-silver transition hover:text-white">
									О НАС
...
const Header: React.FC = () => {
	...
	const headerHeight = useTransform(scrollY, [0, 24], [80, 62]);
		...
		<>
			...
			>
				<div className="mx-auto flex h-full w-full max-w-[96rem] items-center justify-center px-2 md:px-4">
					<motion.div
						className="header-plaque plaque-float group flex h-14 items-center justify-between gap-2 rounded-[32px] px-3 md:px-5"
						style={{ width: "clamp(300px, 85vw, 1180px)", y: yFloat }}
					>
						{/* Left: logo + nav */}
						<div className="flex min-w-0 items-center gap-3 md:gap-6">
							...
							>
								<div className="flex h-10 w-10 items-center justify-center rounded-lg border border-zinc/25 bg-gradient-to-b from-graphite to-charcoal shadow-inner transition group-hover:ring-2 group-hover:ring-olive/25 md:h-11 md:w-11">
								...
								{nav.map((item) => {
									...
									return (
										<a
											...
											className={`relative font-tactical text-[13px] uppercase tracking-[0.08em] md:text-[15px] ${
						...
						<div className="flex items-center gap-1 md:gap-2.5">
							<button
								...
								className="hidden h-8 items-center gap-1.5 rounded-full border border-zinc/20 bg-zinc/10 px-2.5 text-silver transition hover:border-olive/30 hover:bg-zinc/15 focus:outline-none focus:ring-2 focus:ring-olive/40 md:flex md:h-9 md:px-3"
								aria-label="Открыть поиск"
							>
								<Search className="h-3.5 w-3.5 md:h-4 md:w-4" />
								<span className="text-[12px] md:text-[13px]">Поиск</span>
								<span className="font-mono text-[10px] text-silver/60 md:text-[11px]">/</span>
							</button>
							...
							>
								...
								{mockCart.count > 0 && (
									<span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-olive text-[9px] font-bold text-white shadow-[0_0_20px_hsl(var(--olive)_/_0.35)] md:h-[18px] md:w-[18px] md:text-[10px]">
			...
			<style jsx global>{`
				...
				/* Мобильная адаптация для сжатия */
