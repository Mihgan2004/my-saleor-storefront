...
/** Точка-узел таймлайна: хук вынесен на верхний уровень отдельного компонента */
...
/** Карточка, появляющаяся, когда «искра» прошла её порог */
function Feature({
	item,
	threshold, // 0..1 позиция по вертикали
	progress,
}: {
	item: Item;
	threshold: number;
...
export function BenefitsDesktop() {
	...
	useEffect(() => {
		if (inView) {
			const controls = animate(progress, 1, {
				...
				ease: [0.22, 1, 0.36, 1], // мягкий ускоряющийся ход
			});
			return () => controls.stop();
		}
	}, [inView, progress]);

	// равномерные пороги и стороны карточек
	...
	// заполнение линии и позиция «искры»
		...
		<section id="benefits" className="relative bg-black text-white">
			...
			<div className="mx-auto max-w-6xl px-6 py-28">
				<div className="mb-12 text-center">
					<h2 className="text-4xl font-black leading-tight">ПОЧЕМУ GRAYCARDINAL</h2>
				...
				<div ref={ref} className="relative mx-auto h-[72vh] max-h-[900px] min-h-[520px]">
					...
					{/* точки-узлы: теперь без хука внутри .map */}
					{thresholds.map((t, i) => (
